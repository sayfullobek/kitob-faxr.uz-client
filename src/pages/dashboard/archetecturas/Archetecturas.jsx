...
import { ARCHETECTURAS_HEAD } from '../../../utils/TableHeadUtils'
import { DASHBOARD_URL } from '../../../utils/Utils'

export const Archetecturas = () => {
	...
	const getAll = async (page = 1, limit = 10) => {
		try {
			const res = await AutoGet(
				`${APP_API.archetecturas}/all?page=${page}&limit=${limit}`
			)
			setData(res.data)
			setTotalItems(res.total)
			const searchs = await AutoGet(
				`${APP_API.archetecturas}/all?page=1&limit=${res.total}`
			)
		...
		const timer = setTimeout(() => {
			if (search.trim().length === 0) {
				...
				if (searchField != 'projects') {
					filtered = forSearch.filter(item =>
						(item[searchField] || '')
							.toLowerCase()
	...
	const getLabel = () => {
		switch (searchField) {
			...
			case 'projects':
				return 'Loyihasi bo‘yicha qidirish...'
			default:
				return 'Qidirish...'
		}
	}
	...
	return loading ? (
		<Box sx={{ padding: '3rem' }}>
			<Breadcrump
				status={'add'}
				url={`/${DASHBOARD_URL.archetecturasAdd}`}
				arr={ARCHETECTURAS_BREADCRUMP}
			/>
			<Box
				sx={{
					display: 'flex',
			...
			>
				<FormControl sx={{ minWidth: 200 }}>
					...
					>
						...
						<MenuItem value='projects'>Loyihasi</MenuItem>
					</Select>
				</FormControl>
			...
			<AutoTable
				...
				fields={ARCHETECTURAS_HEAD}
				url={DASHBOARD_URL.archetecturasUpdate}
				deleteUrl={APP_API.archetecturas}
				getAll={getAll}
				pathName={pathName}
