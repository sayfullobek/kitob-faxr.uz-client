...
import { PROJECTS_BREADCRUMP } from '../../../utils/BreadcrumpUtils'
import { PROJECTS_HEAD } from '../../../utils/TableHeadUtils'
...
export const Projects = () => {
	...
	const getAll = async (page = 1, limit = 10) => {
		try {
			const res = await AutoGet(
				`${APP_API.projects}?page=${page}&limit=${limit}`
			)
			setData(res.data)
			setTotalItems(res.total)
			const searchs = await AutoGet(
				`${APP_API.projects}?page=1&limit=${res.total}`
			)
	...
	const getLabel = () => {
		switch (searchField) {
			...
			case 'address':
				return 'Manzili bo‘yicha qidirish...'
			default:
				return 'Qidirish...'
		}
	}
	...
	return loading ? (
		<Box sx={{ padding: '3rem' }}>
			<Breadcrump
				status={'add'}
				url={`/${DASHBOARD_URL.projectsAdd}`}
				arr={PROJECTS_BREADCRUMP}
			/>
			<Box
				sx={{
					display: 'flex',
			...
			<AutoTable
				...
				fields={PROJECTS_HEAD}
				url={DASHBOARD_URL.projectsUpdate}
				deleteUrl={APP_API.projects}
				getAll={getAll}
				pathName={pathName}
